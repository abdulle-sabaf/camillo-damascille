...
\begin{tm}\label{tm:1-3-30}
    给定两个线性无关的向量$A,B$，存在非零向量$\mathbf{n}$与它们都垂直。
\end{tm}
任何平面总由两个线性无关的向量确定。平面$\gamma : \{sA + tB + C \, | \, s, t\in\mathbb{R}\}$，
...
考虑两条异面直线，它们的方向向量线性无关。因此，我们也可以用定理\ref{tm:1-3-30}讨论与它们都垂直的直线。
...
给定平面，总有与它垂直的直线。反过来，给定一条直线，是否有与它垂直的平面呢？设直线的方向向量为$\mathbf{n}$，我们来看与它垂直的向量有什么共同特征。
...
既然垂直于同一直线的平面相互平行或重合，那么，相互平行的平面，法向量应该相同。
...
\begin{xt}
    ...
    \indent 4. 求异面直线$\mathbb{R}(0,1,1)+(1,-1,0)$、$\mathbb{R}(1,0,-1)+(0,1,-1)$的公垂线及距离。\\
    \indent 5. 求过$(1,0,0)$且垂直于$\mathbb{R}(0,1,0)+(1,-1,1)$的平面。\\
    \indent 6. 求平行平面$x-y+z=1$和$x-y+z=-1$的距离。
\end{xt}
