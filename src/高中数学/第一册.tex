...
再来看$f_{\mathbb{Q}}$和$f_{\mathbb{R}}$。
\begin{et}
    求函数$f_{\mathbb{Q}}:x\mapsto 2^x$在$x=0$处的极限。
\end{et}
\begin{so}
    按照定义，我们考虑任意一个趋于$0$的有理数列$\{x_n\}_{n\in\mathbb{Z}^+}$。考察数列$\{2^{x_n}\}_{n\in\mathbb{Z}^+}$。我们猜测它收敛到$2^0=1$。
    按照定义，对任何正整数$N$，    由于$\frac{1}{N}>0$，存在正整数$n$，使得$m>n$时总有$|x_m| < \frac{1}{N}$，即$2^{-\frac{1}{N}} < 2^{x_m} < 2^\frac{1}{N}$。
    如果能证明数列$\{2^\frac{1}{n}\}_{n\in\mathbb{Z}^+}$收敛到$1$，那么$\{2^{-\frac{1}{n}}\}_{n\in\mathbb{Z}^+}$收敛到$1$的倒数，仍然是$1$。
    因此，对任意$r>0$，总有正整数$N(r)$，使得$2^{-\frac{1}{N(r)}} > 1 - r$、$2^{\frac{1}{N(r)}} < 1 + r$。对于这个正整数$N(r)$，又存在正整数$n$，
    使得$m>n$时总有$2^{-\frac{1}{N}} < 2^{x_m} < 2^\frac{1}{N}$。
    于是，$m$大于$n$时，总有$|2^{x_m} - 1| < r$。这说明数列$\{x_n\}_{n\in\mathbb{Z}^+}$。
    这个性质对任何趋于$0$的数列成立。这说明$0$是函数$f$在$0$处的极限。

    最后证明数列$\{2^\frac{1}{n}\}_{n\in\mathbb{Z}^+}$收敛到$1$。设$a_n = 2^\frac{1}{n}$，则$a_n^n = 2$。计算$\left(1+\frac{1}{n}\right)^n$。
    因此，根据二项式定理：
    \begin{align}
