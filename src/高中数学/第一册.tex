...
作为日常生活中的“长度”抽象得到的概念，实数应当具有这样的性质：我们认为任意的长度都是真实存在的，直尺上不可能有两点之间没有长度；
...
\begin{df}
    ...
    具体来说，给定数列$\{a_n\}$，其元素属于数集$T$，如果对$T$中任意正数$r$，都存在正整数$N$，使得对$\{a_n\}_{n>N}$中任何两项$a_k, a_m$，都有$|a_k - a_m| \leqslant r$，就说数列$\{a_n\}$是\textbf{自敛}的。
\end{df}

顾名思义，数列自敛，指数列的项会逐渐相互靠近，自相收敛到一起。
...
\end{po}
实数的根本性质它刻画了实数作为“测量结果”的本质。
\textbf{实数是人类在自身尺度下对世界的朴素认知的抽象}。

\begin{sk}
    \mbox{}\\
    \indent 1. 我们定义有理数列的极限时，是否用到了实数？\\
    \indent 2. 我们定义有理数列极限的四则运算时，是否用到了实数？
\end{sk}
\begin{xt}
    \mbox{}\\
...
以上我们用自敛有理数列的极限来定义实数，这样定义的实数是否符合我们直观的认识呢？
...
\end{df}
可以证明，这样定义的相等关系是等价关系\footnote{证明见附录一}。也就是说，实数的相等关系和有理数的相等关系是统一的。
...
\end{df}
可以证明，这样定义的大小关系满足以下基本性质\footnote{证明见附录一}：
\begin{enumerate}
    ...
    \item $\forall a, b, c$，如果$a > b$、$b > c$，则$a > c$。
\end{enumerate}
也就是说，实数的大小关系和有理数的大小关系是统一的。

更关键的是定义实数的四则运算。为此，要证明以下结论\footnote{证明见附录一}：
\begin{tm}
    设实数$a,b$分别对应自敛有理数列的等价类$A,B$。
    \begin{itemize}
...
\subsection{数列的确界}
为了方便，我们希望给每个实数对应一个好用的自敛有理数列。为此，我们希望用一类性质良好的数列来代表实数。
...
最后，我们来讨论实数构成的数列，我们用同样的方法定义自敛实数数列和实数数列的极限。可以证明，
\begin{tm}
    自敛的实数数列总有极限，极限为实数。
\end{tm}
而另一方面，有极限的数列总是自敛的。所以，\textbf{实数数列如果有极限，总是实数}。
...
\begin{tm}{\textbf{数列极限的不等关系传递}}\label{tm:a-0-40}
    \begin{enumerate}
        \item 已知数列$\{a_n\}$收敛到$x$，它的项总小于等于某个数$u$，则$x$小于等于$u$。
        \item 已知数列$\{a_n\}$收敛到$x$，它的项总小于某个数$u$，则$x$小于等于$u$。
        \item 已知数列$\{a_n\}$收敛到$x$，它的项总大于等于某个数$u$，则$x$大于等于$u$。
        \item 已知数列$\{a_n\}$收敛到$x$，它的项总大于某个数$u$，则$x$大于等于$u$。
    \end{enumerate}
\end{tm}
\begin{proof2}
    我们使用反证法证明这几个命题。\\
...
\begin{tm}\label{tm:a-1-2}
    自敛数列经过四则运算仍然是自敛有理数列\footnote{除法中作为被除数的数列的绝对值数列有大于$0$的下界}。
\end{tm}
\begin{proof2}
...
\end{tm}
\begin{proof2}
    自反性：对任意实数$a$，设$a$为自敛有理数列$\{a_n\}$的极限。$\{a_n - a_n\}$是零数列，因此趋于$0$。这说明$a = a$。\\
    对称性：对任意实数$a, b$，设$a$为自敛有理数列$\{a_n\}$的极限，$b$为自敛有理数列$\{b_n\}$的极限。
    记$u_n = a_n - b_n$。那么，$a = b$也就是说$\{u_n\}$趋于$0$。因此$\{b_n - a_n\} = \{-u_n\}$也趋于$0$，因此$b = a$。\\
    传递性：对任意实数$a, b$，设它们分别是自敛有理数列$\{a_n\}, \{b_n\}, \{c_n\}$的极限。如果$a=b$、$b=c$，
    就是说数列$\{a_n - b_n\}$、$\{b_n - c_n\}$趋于$0$，因此它们的和数列趋于$0+0=0$。而它们的和数列是$\{a_n - c_n\}$。这说明$a=c$。
\end{proof2}
极限相互相等的自敛有理数列构成一个集合，每个自敛有理数列恰属于一个集合。这些集合构成所有自敛有理数列的集合的分划。
我们说每个集合是一个等价类。每个实数恰好对应一个等价类，每个等价类也恰好对应一个实数。因此我们可以用这些等价类来表示实数。
实数$a$对应的等价类里每个趋于$a$的自敛有理数列都是$a$的一种表示形式，
就好比$\frac{2}{4}$和$\frac{3}{6}$都是同一个有理数的表示形式一样。

\begin{tm}\label{tm:a-1-15}
    实数的不等关系满足：
    \begin{enumerate}
        \item 自背性：$\forall a$，$a < a$不成立。
        \item 三分律：$\forall a, b$，$a < b$，$a > b$，$a = b$恰有一个成立。
        \item 传递性：$\forall a, b, c$，如果$a < b$、$b < c$，则$a < c$。如果$a > b$、$b > c$，则$a > c$。
    \end{enumerate}
\end{tm}
\begin{proof2}
...
\end{proof2}
注意：我们没有直接证明反对称性：“$a<b$当且仅当$b>a$”。这个结论很有用，可以从以上三个性质推出。
...
\begin{proof2}
    ...
    对任意$\{a_n\}\in A,\,\,\{b_n\}\in B$，根据定理\ref{tm:a-1-2}，数列$\{a_n\}, \{b_n\}$的和、差、积、商（$b\neq 0$）都是自敛有理数列。
    ...
    下面给出证明：\\
    按照等价类的定义，有$\{a_n - a_n'\}$和$\{b_n - b_n'\}$趋于$0$。\\
    \textbf{和差}：注意到$a_n \pm b_n - (a_n' \pm b_n') = (a_n - a_n') \pm (b_n - b_n')$，因此都趋于$0$。\\
    ...
    证明了实数的四则运算自洽，我们接着来证明\textbf{实数的四则运算满足运算律}。\\
    ...
    按照定义，$A_0$的元素总与$A$的某个元素相差一个无穷小，因此也是$A$的元素。反之，$A$中元素总可以写成自身与零数列的和，而零数列趋于$0$，
    因此$A$中元素总是$A_0$的元素。这说明$A = A_0$，即$a + 0 = a$。同理可证$0+a=a$。\\
    (2). 考虑$a\cdot 1$对应的等价类：$A_0 = \{\{a_n \cdot b_n\} \,|\, \{a_n\}\to a, \,\, \{b_n\} \to 1 \}$。
    ...
    反之，$A$中元素$\{a_n\}$总可以写成$\{a_n\cdot 1\}$，因此是$A_0$的元素。这说明$A = A_0$，即$a \cdot 1 = a$。同理可证$1 \cdot a = a$。\\
    (3). 考虑$a - b + b$等价类中的元素，它能写成形如$\{a_n - b_n + b_n'\}$的数列，其中$\{a_n\}$收敛到$a$，$\{b_n\}$、$\{b_n\}$收敛到$b$。
    因此$\{b_n - b_n'\}$收敛到$0$，$\{a_n - b_n + b_n'\}$与$\{a_n\}$相差一个无穷小，因此收敛到$a$。反之，收敛到$a$的数列$\{a_n\}$总能写成$\{a_n - b_n + b_n\}$的形式，
    其中$\{b_n\}$是任意收敛到$b$的数列。这说明$a - b + b = a$。同理可证$a + b - b = a.$\\
    (4). 考虑$a \div b \times b$等价类中的元素，他能写成形如$\{a_n \div b_n \times b_n'\}$的数列，其中$\{a_n\}$收敛到$a$，$\{b_n\}$、$\{b_n\}$收敛到$b$。
    ...
    反之，收敛到$a$的数列$\{a_n\}$总能写成$\{\frac{a_n b_n}{b_n}\}$的形式，
    其中$\{b_n\}$是任意收敛到$b$的数列。这说明$a \div b \times b = a$。同理可证$a\times b \div b = a.$\\
\end{proof2}
...
\begin{proof2}
    ...
    由于$a\neq b$，$c = c$，按照定义，存在有理数$r>0$，使得对任何正整数$m$，都有$n>m$，$a_n - b_n < -r$；而对于$\frac{r}{2}>0$，存在正整数$N$，
    ...
    小于等于关系实际是“小于或等于”的缩写，因此只要验证以上结论对等于关系成立即可\footnote{唯一需要注意的是“反号”不涉及等于关系。}。
\end{proof2}

\begin{tm}\label{tm:a-1-40}
    实数的四则运算有良好的定义，并且和整数、有理数的四则运算一致。
\end{tm}
\begin{proof2}
...
% \begin{tm}\label{tm:a-1-40}
%     任意实数$a$总介于有理数之间：$\exists \,\, U, V\in \mathbb{Q}, \,\,\, U < a < V.$
% \end{tm}
% \begin{proof2}
%     取以$a$为极限的自敛有理数列$\{a_n\}$。定理\ref{tm:a-1-10}说明$\{a_n\}$是有界数列，即有$u, v$使得
%     $$\forall \,\, n\in\mathbb{Z}^+, \,\,\, u < a_n < v.$$
%     根据定理\ref{tm:a-0-40}，$u \leqslant a \leqslant v$，因此，取$U=u-1$，$V=v+1$，就有$U < a < V$。
% \end{proof2}

% \begin{tm}\label{tm:a-1-42}
%     实数与有理数之间可以进行加减法与乘法。定义：
%     \begin{enumerate}
%         \item 实数$a$与有理数$q$的和（差）为等价类$a \pm q = \{\{a_n\pm q\} \, | \, \{a_n\}\to a\}$、
%         $a \pm q = \{\{q\pm a_n\} \, | \, \{a_n\}\to a\}$。
%         \item 实数$a$与有理数$q$的乘积为等价类$a \cdot q = \{\{a_n\cdot q\} \, | \, \{a_n\}\to a\}$、
%         $q \cdot a = \{\{q\cdot a_n\} \, | \, \{a_n\}\to a\}$。
%     \end{enumerate}
%     实数与有理数加减法和乘法与有理数的加法和乘法一致，满足加法结合律、加法交换律、乘法结合律、乘法交换律以及乘法对加法的分配律。
% \end{tm}
% \begin{proof2}
%     首先证明定义自洽。$\{\{a_n\pm q\} \, | \, \{a_n\}\to a\}$的任两个元素$\{a_n\pm q\}$与$\{b_n\pm q\}$的差就是$\{a_n - b_n\}$，即无穷小，
%     所以它是等价类。$\{\{q\pm a_n\} \, | \, \{a_n\}\to a\}$的任两个元素$\{q\pm a_n\}$与$\{q\pm b_n\}$的差就是$\{\mp(a_n - b_n)\}$，即无穷小，所以它是等价类。
%     $\{\{a_n\cdot q\} \, | \, \{a_n\}\to a\}$中任两个元素$\{a_n\cdot q\}$与$\{b_n\cdot q\}$的差是$\{q(a_n - b_n)\}$，因此是无穷小，所以它是等价类。
%     同理$\{\{q\cdot a_n\} \, | \, \{a_n\}\to a\}$是等价类。\\
%     其次证明一致性：\\
%     (1). $q = 0$时，$\{a_n\pm q\}$就是$\{a_n\}$，所以$a+0=a$，同理$0+a=a$。\\
%     (2). $q = 1$时，$\{a_n\cdot q\}$就是$\{a_n\}$，所以$a\cdot 1=a$，同理$1\cdot a=a$。\\
%     接下来证明运算律：\\
%     (1). $\{a_n + q\} = \{q + a_n\}$总成立，所以$a + q = q + a$，加法交换律成立。\\
%     (2). $\{a_n \cdot q\} = \{q \cdot a_n\}$总成立，所以$a \cdot q = q \cdot a$，加法交换律成立。\\
%     (3). 设$a$为实数，$p, q$为有理数$\{(a_n + p) + q\} = \{a_n + (p + q)\}$总成立，所以$(a + p) + q = a + (p + q)$。
%     同理，$(p + a) + q = p + (a + q)$、$(p + q) + a = p + (q + a)$。\\
%     (4). 设$a$为实数，$p, q$为有理数$\{(a_n \cdot p) \cdot q\} = \{a_n \cdot (p \cdot q)\}$总成立，所以$(a \cdot p) \cdot q = a \cdot (p \cdot q)$。
%     同理，$(p \cdot a) \cdot q = p \cdot (a \cdot q)$，$(p \cdot q) \cdot a = p \cdot (q \cdot a)$。\\
%     (5). 设$a$为实数，$p, q$为有理数$\{(a_n + p) \cdot q\} = \{a_n \cdot q + p \cdot q)\}$总成立，所以$(a + p) \cdot q = a \cdot q + p \cdot q$。
%     同理，$(p + q) \cdot a = p \cdot a + q \cdot a$。
% \end{proof2}

% \begin{tm}\label{tm:a-1-45}
%     实数与有理数的加减法和乘法，与实数的不等关系兼容。不等式两边加减相同的数，不等号不变。不等式两边乘以相同的数，
%     正数则不等号不变，负数则不等号反号。
% \end{tm}
% \begin{proof2}
%     对任意两个数列$\{a_n\}$、$\{b_n\}$，$\{a_n\pm q\}$与$\{b_n\pm q\}$的差，等于$\{a_n\}$与$\{b_n\}$的差。所以，只要后者自某项往后总大（小）于零，
%     前者自某项往后也总大（小）于零。这说明不等式两边加减相同的有理数，不等号不变。\\
%     对任意实数$a$，设两个数列$\{a_n\}$、$\{b_n\}$都收敛到$a$。设有理数$p<q$，考虑$a_n + p - (b_n + q)$。
%     $$ a_n + p - (b_n + q) = (a_n - b_n) + p - q. $$
%     按照定义$\{a_n - b_n\}$趋于$0$，所以存在自然数$N$，只要$n>N$，就有$ a_n + p - (b_n + q) < 0$。这说明$a + p < a + q$。
%     同理可推出其他不等情况：有理数不等式两边加减相同的实数，不等号也不变。\\
%     对任意两个数列$\{a_n\}$、$\{b_n\}$，$\{a_n\cdot q\}$与$\{b_n\cdot q\}$的差，等于$\{a_n\}$与$\{b_n\}$的差的$q$倍。所以，如果$q>0$，
%     那么只要后者自某项往后总大（小）于零，前者自某项往后也总大（小）于零；如果$q<0$，那么只要后者自某项往后总大（小）于零，前者自某项往后也总小（大）于零。
%     这说明不等式两边乘以相同的有理数，正数则不等号不变，负数则不等号反号。\\
%     有理数不等式两边乘以相同的实数，正数则不等号不变，负数则不等号反号。
% \end{proof2}