...
\begin{align}
    \operatorname{Lian} : \,\,\, \mathbb{R}^\mathbb{N} &\longrightarrow \mathbb{R} \notag \\
    \{a_n\} &\mapsto a_\infty \notag
\end{align}
...
\begin{sk}
    \mbox{}\\
    \indent 1. 自敛数列定义中的“$|a_k -a_m| \leqslant r$”能否改成“$|a_k -a_m| \leqslant r$”？两种定义是否有区别？\\
...
\chapter{实变函数中的极限}
第三章中，我们学习了数列的极限。数列是正整数集上的映射。下面，让我们回到实变函数的研究中来。
数列的极限，对研究实数集上的映射，有什么作用呢？
...
\section{实变函数在一点的极限}
为了用数列的极限来研究实变函数，我们首先给出用数列的极限描述任一个实数的方法。首先给出可以用有理数任意逼近任意实数的根据：
·\begin{tm}
    总存在与给定实数任意接近的有理数。具体来说，给定实数$a$，对任何有理数$r>0$，总存在有理数$q$，使得$|a - q| < r$。
\end{tm}
也就是说，对任意正整数$n$，都可以找到有理数$q_n$与$x$的差小于$\frac{1}{n}$。即数列$\{\frac{n_m}{m}\}_{m\in\mathbb{Z}^+}$的极限就是$x$。
...
从直观上看，这说明$f_{\mathbb{Q}}$在平面中的曲线在$0$点附近没有“突变”或“跳跃”。无论自变量$x$以什么方式靠近$0$，点$(x,\,\,f_{\mathbb{Q}}(x))$
都不会在$y$轴方向出现突然的变动。这符合我们对$f_{\mathbb{Q}}$曲线的粗略认知。
...
\begin{df}\textbf{实变函数在一点的极限}
    ...
    （$a_n$总在$f$定义域中），就趋于同一个极限$y$，那么就说$y$是函数$f$在自变量等于$a$时的\textbf{极限}\footnote{也称$f$在$a$处的极限。}，
    或者说自变量趋于$a$时，$f$趋于或\textbf{收敛到}$y$。
\end{df}
注意：从定义可以看出，我们不要求$f$在$a$处有定义。
...
具体来说，如果函数$f$在实数$a$处有极限$y$，那么当$f$图像上一点$x$的横坐标与$a$足够接近时，
它的纵坐标应该和$y$足够接近。最终，函数图像“应该经过”点$(a,\,\, y)$。
这种说法并不严谨，但可以给我们一点直觉上的帮助。

最后我们来介绍函数在一点极限的另一种表述。这种表述不使用数列的概念，而使用区间的概念。具体来说，是一个叫做“邻区间”的概念。

如前所述，直观上来说，函数在一点有极限，就是当$f$图像上一点$x$的横坐标与$a$足够近时，它的纵坐标应该和$y$足够接近。我们把“近”这个概念用“邻区间”来刻画。

\begin{df}{\textbf{邻区间}}
    给定数$a$和正数$r>0$，我们把数轴上$a-r$与$a+r$为端点的区间称为$a$的（半径为$r$的）\textbf{邻区间}。
    $(a-r,\,\,a+r)$称为$a$的\textbf{开邻区间}，$[a-r,\,\,a+r]$称为$a$的\textbf{闭邻区间}，分别记为$U(a,r)$和$\overline{U}(a,r)$\footnote{不至于混淆时，可以简记为$U$和$\overline{U}$。下同。}。
    
    把$a$从其中“挖去”，剩余的部分称为$a$的\textbf{去心邻区间}。
    $(a-r,\,\,a)\cap(a,\,\,a+r)$称为$a$的\textbf{去心开邻区间}，$[a-r,\,\,a+r]$称为$a$的\textbf{去心闭邻区间}，分别记为$\overset{\circ}{U}(a,r)$和$\overset{\circ}{\overline{U}}(a,r)$。
\end{df}
直观上看，$a$的半径为$r$的邻区间就是以点$a$为中心，两边各延伸$r$得到的区间，而去心邻区间就是把作为中心的点$a$挖掉了。
...
\begin{tm}{\textbf{数列极限的不等关系传递}}\label{tm:a-0-45}
...
数列之间的不等关系可以通过极限传递，但极限只描述数列的大体行为，不描述数列具体的项的性质。因此，我们可以设想这样的情景：
...
\begin{sk}
    \mbox{} \\
    \indent 1. 对比用整数重新定义有理数和用有理数重新实数的过程。为什么重新定义实数的过程步骤明显更多？\\
