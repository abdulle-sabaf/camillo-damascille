...
\chapter{实变函数中的极限}
上一章中，我们学习了数列的极限。数列是正整数集上的映射。下面，让我们回到实变函数的研究中来。
...
\end{et}
\begin{so}
    按照定义，我们考察任意一个趋于$0$的数列$\{x_n\}_{n\in\mathbb{Z}^+}$。按照极限的运算法则，数列$\{x_n^2\}_{n\in\mathbb{Z}^+}$也趋于$0$。
...
再来看$f_{\mathbb{Q}}$和$f_{\mathbb{R}}$。设$a$为有理数。有理数列$\{a_n\}_{n\in\mathbb{Z}^+}$趋于$a$的时候，
